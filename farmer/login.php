...
$Farmer_type = substr(strtoupper($un),0,2);
$unique = substr($un,2);



$qr="SELECT uid, password FROM farmersignup WHERE uid='$unique' and password=SHA('$pass')";

	$quer=mysqli_query($con,$qr)or die("error in insertion");

	$count=mysqli_num_rows($quer);

	if (($count) And ($Farmer_type == "FA"))
{

	
  
	$qrcheck="SELECT * FROM farmersignup WHERE uid='$unique' and password=SHA('$pass')";
	$quercheck=mysqli_query($con,$qrcheck)or die("error in insertion in farmer session signup page");
  $rowcheck=mysqli_fetch_array($quercheck);
    $_SESSION['unique_id'] = $rowcheck['uid'];                  //unique id number
  $_SESSION['type'] = $rowcheck['type']; 
       
  
  echo '<p><script type="text/javascript"> alert("You have been logged in successfully");
        location="indexfa.php"
        </script>in</a>.</p>';  	
}
 else
{  
...
<div class="form1 container"><!--login-up Form Starting-->
	...
	<p class="logh2">Farmer Login</p>
		<table class="table1"><!--table started-->
	<!--username started-->
	<tr>
	<td><label for="uid">Login ID</label><br>
	<input type="text" name="uid" id="uid" size="30" value="<?php echo $un?>"  placeholder="यूनिक आईडी / मोबाइल नंबर" required oninvalid="this.setCustomValidity('कृपया अपना यूनिक आईडी / मोबाइल नंबर यहाँ रिक्त स्थान में भरें')"
...
<a href="http://krishi.rf.gd/farmer/su.html" class="hre2">Create your account here?</a>
